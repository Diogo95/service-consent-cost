...
import com.apps.test.data.CMPGatewayImplementation
import com.apps.test.data.ServicesCost
import com.apps.test.data.converter.ServiceDataObjectConverter
...
class MainActivity : AppCompatActivity() {
    ...
    private val textViewCost by lazy { findViewById<TextView>(R.id.text_view_cost) }
    ...
    private val serviceCostManager by lazy {
        ServiceCostManager(
            RetrieveCostService(
                CMPGatewayImplementation(
                    ServiceDataObjectConverter()
                ),
    ...
    private fun handleUserResponse(userResponse: UsercentricsConsentUserResponse?) {
        ...
        val costs = calculateServicesCost(userResponse.consents, userResponse.userInteraction)
        showServicesCost(costs)
    }

    private fun calculateServicesCost(
        consents: List<UsercentricsServiceConsent>,
        userInteraction: UsercentricsUserInteraction
    ) = serviceCostManager.serviceCost(userInteraction, consents)

    private fun showServicesCost(cost: CostPresenter){
        println(cost.servicesCosts)
        textViewCost.text = "Total Cost: ${cost.totalCost}"
    }
